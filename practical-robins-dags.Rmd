---
title: 'Practical: Causal diagrams and conditioning scenarios'
author: "Tom Palmer"
# date: "19/20"
output:
  html_document:
...
* Estimating the marginal odds ratio for the effect of $E$ on $D$ (i.e. use $D$ as the outcome/dependent variable and $E$ as the covariate)

```{r}
mod1 <- glm(d ~ e, family = binomial, data = dat)
summary(mod1)
round(exp(cbind(coef(mod1), confint.default(mod1))), digits = 2)
```

* Estimating the conditional odds ratio for the association between $E$ and $D$ adjusting for $E^*$

```{r}
mod2 <- glm(d ~ e + es, family = binomial, data = dat)
summary(mod2)
round(exp(cbind(coef(mod2), confint.default(mod2))), digits = 2)
```

* Showing that you get equivalent ORs in both subgroups
...
summary(mod3)
round(exp(cbind(coef(mod3), confint.default(mod3))), digits = 2)
```
